...
data_week = readRDS('./data/weekly_data.RDS')         # 642,205 obs - removed Feb 29 observations for simplicity in week analyses
                                                          # 717 observations on Feb 29 across years
...
table_1_data_pre %>%
  ...
  geom_violin(alpha = .4) +
  stat_summary(fun = median, geom = "point", color = "IndianRed", size = 2) +
...
climate_95_pctiles =
  ...
  # See note below re: area-measurement vs. Division-measurement
  ...
  mutate(
    ...
    # Upon second thought, doesn't make sense to esimate heat wave days on a Division scale - if we want to make inference on a Division scale, we should estimate with as much granularity as possible at first (i.e. area/upazila-level), then scale inference up to Division
      # Also, from a practical/logical standpoint, climate measures were measured at the area level, so District-based heatwave cutoffs wouldn't make sense
    
    # heat_wave_day_division = case_when(tmax >  tmax_95_division & tmin >  tmin_95_division ~ 1,
    #                                    tmax <= tmax_95_division | tmin <= tmin_95_division ~ 0,
    #                                    TRUE ~ 999),
    heat_wave_day_area = case_when(tmax >  tmax_95_area & tmin >  tmin_95_area ~ 1,
                                   tmax <= tmax_95_area | tmin <= tmin_95_area ~ 0),
    heat_wave_day_area = as.logical(heat_wave_day_area),
    # heat_wave_day_difference = case_when(heat_wave_day_division - heat_wave_day_area == 0 ~ 0,
    #                                      heat_wave_day_division - heat_wave_day_area != 0 ~ 1,
    #                                      TRUE ~ 999)
    precip_wave_day_area = case_when(precip >  p_95_area ~ 1,
                                     precip <= p_95_area ~ 0),
    
    precip_wave_day_area = as.logical(precip_wave_day_area)
  ) %>% 
    
  # Week-level climate predictors
  group_by(year, surv_area, week) %>% 
  mutate(
    
     # Week-grouped measures don't make as much sense - need 7-day window function
        # But need them for weekly timeseries
    heat_wave_day_area_week = sum(heat_wave_day_area),

    precip_wave_day_area_week = sum(precip_wave_day_area)
  ...
  mutate(
    ...
    heat_wave_day_area_7days = zoo::rollsum(heat_wave_day_area, k = 7, na.pad = TRUE, align = "right"),
    
    precip_wave_day_area_7days = zoo::rollsum(precip_wave_day_area, k = 7, na.pad = TRUE, align = "right")

   # Number of 'heat wave days':              # Number of 'extreme precip days'"
    # Days above both 95%ile tmax and tmin      # Days above 95%ile precipitation
         # heat_wave_day_area                   precip_wave_day_area
         # 0:135024                             0:129759            
         # 1:  1574                             1:  6839 

   # Number of heat wave / ex pr days in the same week - *** doesn't make much sense - should do running sum of past 7 days instead
   # UPDATED to rolling sum of past 7 days:
            
  # heat_wave_day_area_7days      n                    precip_wave_day_area_7days     n
  #                 <int>  <int>                                        <int>  <int>
  #                     0 130018                  1                         0 105523
  #                     1   3858                  2                         1  19240
  #                     2   1290                  3                         2   7604
  ...
  # (*** 6 NAs for each of 22 areas (6*22=132) - window function can't capture 6 days of data before Jan 1 1990)
  ...
  mutate(
   ...
   # Flag for a day during a heat wave - during a period of 3+ consecutive heat wave days  
   heat_wave_area = case_when(heat_wave_day_area == 1 & lead(heat_wave_day_area, 1) == 1 & lead(heat_wave_day_area, 2) == 1 ~ 1,
                              heat_wave_day_area == 1 & lag(heat_wave_day_area, 1) == 1 & lead(heat_wave_day_area, 1) == 1 ~ 1,
                              heat_wave_day_area == 1 & lag(heat_wave_day_area, 2) == 1 & lag(heat_wave_day_area, 1) == 1 ~ 1,
                              TRUE ~ 0
   )
 
  ) %>% ungroup()

# Checking heat wave variable
climate_95_pctiles %>% 
  filter(heat_wave_area == 1) %>% 
  select(surv_area, date, starts_with(c("tmax", "tmin")), heat_wave_day_area, starts_with("heat_wave_area")) %>% 
  arrange(surv_area, date) # %>% 
  # view()

# Getting heat wave lengths - idea from https://stackoverflow.com/questions/15197133/get-sum-of-consecutive-day-values
heat_rle = rle(climate_95_pctiles$heat_wave_area)
ival = heat_rle$lengths %>% as_tibble()
# my own code - using uncount() to repeat each cell by its value
heat_count = ival %>% mutate(value2 = value) %>% uncount(value2)

climate_95_pctiles =
  bind_cols(climate_95_pctiles, heat_count) %>% 
  mutate(value = as.numeric(value)) %>%   # was integer, need double
  select(everything(), heat_wave_length = value) %>% 
  mutate(
    heat_wave_length = case_when(heat_wave_area == 0 ~ 0,
                                 heat_wave_area == 1 ~ heat_wave_length)
  )

# Cleaning up
# rm(heat_rle, ival, heat_count)
...
table_1_data =
  ...
  # mutate(
  #   year = as.numeric(year) # was having issues joining
  # ) %>% 
  
  # Joining with new ones
  left_join(
    .,
    climate_95_pctiles %>% select(surv_area, date, p_95_area:heat_wave_length),
    by = c("surv_area", "date")
  ) %>% 
...
# Past-7-day heat wave DAY exposure (95%ile for tmax AND tmin for one day, regardless of continuity)
...
label(table_1_data$heat_wave_day_area_7days) = "Heat wave days in prior 7 days (max. and min. temp. > 95th %ile)"
heat_wave_day_table =
  merge_table_ones(heat_wave_day_area_7days)

```

```{r}

# Past-7-day extreme precipitation exposure (95%ile for precip for one day, regardless of continuity)
...
label(table_1_data$precip_wave_day_area_7days) = "Extreme precipitation days in prior 7 days (precipitation > 95th %ile)"
precip_wave_day_table =
  merge_table_ones(precip_wave_day_area_7days)
...
table_1 =
  ...
  cols_label(label = md("")) %>%
  tab_header(
    title = "Table 1. Demographic and exposure profile of study population"
  ) %>% 
  tab_options(
    table.font.size = '80%'
  )
...
# Saving
# saveRDS(table_1, './figures/table_1.RDS')

```

Creating a separate heat wave table - overall heat waves during survey period, by region

```{r}
  
# Getting heat wave lengths - idea from https://stackoverflow.com/questions/15197133/get-sum-of-consecutive-day-values
heat_rle = rle(climate_95_pctiles$heat_wave_area)
ival = heat_rle$lengths %>% as_tibble() %>% rename(lengths = value)
ival2 = heat_rle$values %>% as_tibble() %>% rename(values = value)
ival3 = tibble(
  
  lengths = heat_rle$lengths,
  values = heat_rle$values
  
)
# my own code - using uncount() to repeat each cell by its value
heat_count = ival3 %>% mutate(a = lengths) %>% uncount(a)

heat_waves_by_area = 
  bind_cols(heat_count, 
            climate_95_pctiles %>% select(surv_area, date, year, month, day)) %>% 
  filter(values == 1) %>% 
  unique() %>% 
  mutate(date_lag = date - lag(date),
         date_lag = as.numeric(date_lag),
         date_lag = case_when(is.na(date_lag) ~ 0, 
                              TRUE ~ date_lag)) %>%    ## if difference between date in current cell and previous cell is not 1 day, then it's the first of a series - keep only these to count heat waves
  filter(date_lag != 1) %>% 
  select(heat_wave_length = lengths, everything(), -date_lag, -values)

heat_wave_freq =
  heat_waves_by_area %>% 
  mutate(heat_wave_length = as.factor(heat_wave_length)) %>% 
  left_join(., table_1_data %>% select(surv_area, area_name) %>% unique(), by = "surv_area") %>% 
  ggplot(aes(x = heat_wave_length)) +
  geom_bar() +
  facet_wrap(~ area_name) +
  scale_x_discrete() + 
  labs(x = "Heat wave length (days)",
       y = "Frequency",
       title = "Frequency of heat wave by surveyed area and length of heatwave")

# Saving to figures
# saveRDS(heat_wave_freq, './figures/heat_wave_freq.RDS')

```

# FINAL DATASET - INDIVIDUAL LEVEL

```{r}

# saveRDS(table_1_data, './data/final_dataset_individual.RDS')

# However, in order to do a lagged dataset, I need ALL heatwave days, even if they are matched with NA wasting data
  # Create in ENACTS dataset
  # Join with daily and/or weekly dataset

```

# Also saving climate 95th percentiles file

```{r}

# saveRDS(climate_95_pctiles, './data/climate_95_pctiles.RDS')
...
Next steps:
  ...
  * xxx Create weekly dataset with weekly exposure counts
  * xxx Add to Table 1 rows
    + xxx Merge Table 1
  * Start on figures
  ...
  * Create heat wave (3+ days) variable using enacts.RDS
  * Weekly climate measures:
    + xxx Heat wave + length
    + Experienced heat wave in past week?
    + xxx Past-seven-day # extreme heat days
    + xxx Past-seven-day # extreme precipitation days
